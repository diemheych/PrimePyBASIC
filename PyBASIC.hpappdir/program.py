        ...
        for line_num in line_numbers:
            ...
            if statement and len(statement) > 0:
                if statement[0].category == Token.WHILE:
                    ...
                    if not while_stack:
                        raise RuntimeError("WEND without matching WHILE at line "+line_num)
                    while_stack.pop()

        if while_stack:
            raise RuntimeError("WHILE without matching WEND at line "+while_stack[0])
            ...
            if new_num in line_numbers and new_num not in lines_to_renumber:
                ...
                for ln in line_numbers:
                    if ln == new_num and ln not in lines_to_renumber:
                        raise ValueError("New line number "+new_num+" conflicts with existing line "+l)
